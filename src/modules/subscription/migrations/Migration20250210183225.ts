...
export class Migration20250210183225 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table if not exists "plan" ("id" text not null, "name" text not null, "slug" text not null, "customer_group_name" text not null, "meals_per_week" integer not null, "meals_per_day" integer not null, "price_per_meal" numeric not null, "category_id" text not null, "delivery_schedule_id" text not null, "raw_price_per_meal" jsonb not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "plan_pkey" primary key ("id"));');
    this.addSql('CREATE UNIQUE INDEX IF NOT EXISTS "IDX_plan_slug_unique" ON "plan" (slug) WHERE deleted_at IS NULL;');
    this.addSql('CREATE UNIQUE INDEX IF NOT EXISTS "IDX_plan_customer_group_name_unique" ON "plan" (customer_group_name) WHERE deleted_at IS NULL;');
