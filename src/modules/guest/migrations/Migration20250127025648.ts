...
export class Migration20250127025648 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table if not exists "guest" ("id" text not null, "current_stage" text not null default \'initial\', "phone_number" text null, "phone_verified" boolean not null default false, "email" text null, "password" text null, "name" text null, "last_name" text null, "address" text null, "city" text null, "expires_at" timestamptz not null, "last_active_at" timestamptz not null, "token" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "guest_pkey" primary key ("id"));');
  ...
  async down(): Promise<void> {
    ...
    this.addSql('alter table if exists "utm" drop constraint if exists "utm_guest_id_foreign";');
